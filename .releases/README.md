...
**Event behavior:**
- **Pre-release created/published** (`prereleased` or `published` while still a pre-release): **full action** — adds/updates the new version in `releases.properties` on the version PR's branch (and mirrors to `main`; a new entry re-runs the branch tests), then dispatch to quickpick.
...
- **Release promoted to a full release** (`released`): **full action** — update `releases.properties` on the PR branch (and mirror to `main`), then dispatch to quickpick. *Caveat:* GitHub does not reliably deliver this event when an existing release is just converted prerelease → latest, so the pipeline never relies on it — the initial `published` full action already keeps `releases.properties` current.
...
On **every** non-PR trigger the workflow then dispatches `module_release_created` to quickpick (`combine_releases.yml`), **even when `releases.properties` did not change** (commit skipped) — the quickpick combine is idempotent and re-runs so the combined release always reflects current data. Only a hard failure of the `update-properties` job suppresses the dispatch.
...
**Solution:** The workflow subscribes to `published`, `prereleased`, `released`, `edited`, and `deleted` (the `released`/`prereleased` types are best-effort — GitHub does not reliably deliver them, so the pipeline never relies on them). A pre-release adds/updates its entry to `releases.properties` on the version PR's branch, an edit (files changed) runs the full action (re-checking the stored URL/version), and a deletion purges the release's entries from `releases.properties`. Because the `published` full action already records a pre-release's version, converting it later to latest is a **no-op for `releases.properties`** — if a combine is still needed, run the workflow manually.
